...
namespace Lab5.App_Code
{
    ...
    public class Calendar
    {
        ...
        public void loadAllEvents(int year, int month)
        {
            ...
            SqlCommand command = new SqlCommand("SELECT EventMemoryId, UserFK, EventName, EventDescription, Scheduled FROM EventMemory WHERE Scheduled <= @upper AND Scheduled >= @lower;", connection);
            try
            {
        ...
        public void loadUserEvents(int year, int month, object UserId)
        {
            ...
            DateTime upper = new DateTime(year, month, DateTime.DaysInMonth(year,month),23, 59, 59, 99);
            SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["SqlSecurityDB"].ConnectionString);
            SqlCommand command = new SqlCommand("SELECT EventMemoryId, UserFK, EventName, EventDescription, Scheduled FROM EventMemory WHERE UserFK = @UserID AND Scheduled <= @upper AND Scheduled >= @lower;", connection);
            try
            {
